...
def print_aggregated_messages(session_id):
    ...
    time_range = (
        f"{sorted_messages[0]['start']:.2f}s - {sorted_messages[-1]['end']:.2f}s"
    )
...
def webhook():
    if request.method == "POST":
        ...
        if not perplexity_key:
            ...
            return (
                jsonify(
                    {"status": "error", "message": "No perplexity key found for uid"}
                ),
                400,
            )
        ...
        if time_since_last >= AGGREGATION_INTERVAL and message_buffer[session_id]:
            ...
            if intent["intent"].lower() == "yes":
                ...
                return jsonify({"message": f"Perplexity: {response_text}"}), 200
